...
- **诚实边界(未验证项,勿读作已完成)**:
  1. 第三路仍走 `codegraph query` **词法匹配**,不是语义召回 → 命名怪异的资源访问函数会漏。真正的"自由探索探针"(agent 推理式召回)属**重构范畴**,本次未做。
  2. P2-a 是**"先小改验证再决定重构"策略里的小改**:用最便宜的粗召回验证 P1 裁判在真实越权上是否判得准,再决定是否投资探针重构。**尚无真实项目数据证明其召回/判定质量**——须先跑一轮试点(呼应 §5.3、§10-D 成本闸门)。
