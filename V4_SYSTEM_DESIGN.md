# Fuzzy Semantic Audit V4 —— 面向 CLI 的审计 Skill 设计
...
**V4 架构的核心思想是：**
> **“以静态算法做极致收缩，以 CLI Skill / 提示词驱动的审计执行器做深度研判”**

V4 不去用昂贵的 AI 扫全库，而是用 V3 静态管道将数十万节点快速预过滤为 10~20 个最高危的控制流 Sink 候选点（Candidate）；随后对每个高危点，由 CLI 触发一个带有“代码检索与调用图探索工具箱”的 **Verifier Skill** 执行器。该 skill 会接收仓库上下文、候选点线索和审计提示词，按固定输出格式完成一次结构化研判，产出完整的调用链传导证据图与可达性结论。
...
┌──────────────────────────────────────────────────┐
│   阶段二：V4 审计 Skill 执行循环 (CLI Skill 引擎)    │
│   - 串行工作流调度器轮询 (orchestrate_audit)       │
│   - CLI 触发 verifier skill 执行审计任务          │
│   - 按提示词与上下文完成代码图回溯与证据整理      │
│   - 产出结构化可达性判定与审计报告               │
└──────────────────────────────────────────────────┘
                         │
                         ▼
...
### 3.2 Skill 执行预算保护（Budget Cap 机制）
*   **盲点**：Skill 在追溯多层嵌套、循环调用（Circular Calls）的调用链时，可能会陷入无休止的代码读取和 callers 跳转循环，导致 Token 消耗爆炸或卡死。
*   **优化**：在 Skill 的工具调用、上下文拼装和推理循环中，强制定义 **“单漏洞分析预算上限”**（例如：最大 API 交互次数 = 12 次，最大累积 Token 数 = 50,000）。一旦达到该阈值，系统自动熔断，强制中断当前 skill 执行，将该候选点标注为 `needs_review` 并输出已探索的局部链路日志。
...
### 📦 模块 4：审计 Skill / 研判执行器 (Verify) —— 对应 `src_v4/verify/`
```python
class AgentTools:
    """提供给 CLI Skill 在审计任务中检索仓库上下文的本地工具箱"""
...
Skill 完成审计后，必须提交一份结构化的漏洞分析报告，并输出最终的 Verdict 判决。[verdict_policy.py](file:///root/fuzzy-semantic-audit/src_v3/verify/verdict_policy.py) 依据 Skill 的报告进行最终状态路由：

| Skill 研判结论 (Verdict) | 可达性证据链 (Evidence Path) | 系统最终路由状态 |
| --- | --- | --- |
...
4.  **Stage 4: P3 阶段 —— Skill 执行与 Workflow 串行集成**
    完成 Skill 工具箱、Verifier Skill 执行流程开发，并由 `AuditOrchestrator` 串联起整个流程，输出漏洞链报告。
